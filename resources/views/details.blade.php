...
<svg class="review-star" viewBox="0 0 9 9" xmlns="http://www.w3.org/2000/svg">
            ...
            <!-- Half filled star -->
            <path fill="gold" mask="url(#half-mask)" d="M4.0172 0.313075L2.91869 2.64013L0.460942 3.0145C0.0201949 3.08129 -0.15644 3.64899 0.163185 3.97415L1.94131 5.78447L1.52075 8.34177C1.44505 8.80402 1.91103 9.15026 2.30131 8.93408L4.5 7.72661L6.69869 8.93408C7.08897 9.14851 7.55495 8.80402 7.47925 8.34177L7.05869 5.78447L8.83682 3.97415C9.15644 3.64899 8.97981 3.08129 8.53906 3.0145L6.08131 2.64013L4.9828 0.313075C4.78598 -0.101718 4.2157 -0.10699 4.0172 0.313075Z" />
        </svg>
                                    @else
                                        {{-- Empty star --}}
                                        ...
                                        @for ($i = 5; $i >= 1; $i--)
                                            <div class="rating-bar d-flex align-items-center mb-2">
                                                ...
                                                <div class="progress flex-grow-1 me-2" style="height: 8px;">
                                                    <div class="progress-bar bg-warning" style="width: {{ $product->rating_percentages[$i] ?? 0 }}%"></div>
                                                </div>
                                                <span class="rating-count text-muted">{{ $product->rating_distribution[$i] ?? 0 }}</span>
                                            </div>
                                        @endfor
                                    </div>
                                </div>
                                    ...
                                    <form id="review-form" data-product-id="{{ $product->id }}">
                                        ...
                                        <div class="select-star-rating mb-4">
                                            ...
                                            <div class="star-rating" id="star-rating">
                                                @for ($i = 1; $i <= 5; $i++)
                                                    <svg class="star-rating__star-icon" width="20" height="20" fill="#ccc" 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadReviews(page = 1) {
        ...
        .catch(error => {
            ...
            loadingEl.style.display = 'none';
            if (page === 1) {
                emptyEl.innerHTML = '<p class="text-muted">Error loading reviews. Please try again later.</p>';
                emptyEl.style.display = 'block';
            }
        })
        .finally(() => {
    ...
    function createReviewHtml(review) {
        ...
        for (let i = 1; i <= 5; i++) {
            if (i <= review.rating) {
                starsHtml += '<svg class="review-star" style="fill: gold;"><use href="#icon_star" /></svg> ';
            } else {
        ...
        .then(data => {
            if (data.success) {
                ...
                if (reviewFormContainer) {
                    reviewFormContainer.innerHTML = '<div class="alert alert-info"><p class="mb-0">You have already reviewed this product.</p></div>';
                }
            } else {
    ...
    function restoreReviewForm() {
        const reviewFormContainer = document.querySelector('.product-single__review-form');
        const alreadyReviewedMessage = reviewFormContainer ? reviewFormContainer.querySelector('.alert-info') : null;
